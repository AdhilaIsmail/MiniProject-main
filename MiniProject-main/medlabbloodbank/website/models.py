...
class UserManager(BaseUserManager):
    def create_user(self, phone, email, password=None, role=None):
        if not email:
            raise ValueError('User must have an email address')
    ...
    def create_superuser(self, phone, email, password=None):
        user = self.create_user(
            ...
            password=password,
            phone=phone,
            role=CustomUser.STAFF,  # Set the role to "staff" for superuser
        )
        user.is_admin = True
        user.is_active = True
        user.is_staff = True
    ...
    def create_hospital(self, phone, email, password=None):
        # Create a hospital user with the "hospital" role
        return self.create_user(
            email=self.normalize_email(email),
            password=password,
            phone=phone,
            role=CustomUser.HOSPITAL  # Set the role to "hospital"
        )
...
class CustomUser(AbstractUser):
    ...
    phone = models.CharField(max_length=12, blank=True)
    password = models.CharField(max_length=128)
    role = models.PositiveSmallIntegerField(choices=ROLE_CHOICE, blank=True, null=True, default=DONOR)
    is_admin = models.BooleanField(default=False)
    is_staff = models.BooleanField(default=False)
    is_active = models.BooleanField(default=True)
    is_superadmin = models.BooleanField(default=False)
